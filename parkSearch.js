...
// Get the element IDs needed for this
const locationDropdown = document.getElementById("locationSelect");
const parkTypeDropdown = document.getElementById("typeSelect");
const searchButton = document.getElementById("searchButton");
const resultsBox = document.getElementById("results");


// Loops through the whole location array so not needed for putting the options within HTML
for (let i = 0; i < locationsArray.length; i++) {
  locationDropdown.innerHTML += `<option value="${locationsArray[i]}">${locationsArray[i]}</option>`;
}

// Empty array will be used for the park type 
const uniqueParkTypes = [];

// Goes through the national parks and gets the park type from the current element in the array  
for (let i = 0; i < nationalParksArray.length; i++) {
  const parkType = nationalParksArray[i].LocationName;
